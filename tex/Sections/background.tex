...
The pervasive digitisation of financial services has generated transaction ecosystems of unprecedented scale and complexity. Classical machine learning approaches, whether logistic regression, support vector machines, or shallow neural networks, operate on tabular feature vectors and treat each data point as an independent, identically distributed observation. This assumption is fundamentally at odds with the relational nature of financial data, where the behaviour of an account cannot be understood in isolation but only in the context of the network of counterparties it interacts with. Graph Machine Learning (GML) addresses this gap by enabling models to operate directly on graph-structured data, learning representations that jointly encode both the attributes of individual entities and the topology of their interactions~\cite{hamilton2020graph}.

The field of GML encompasses a broad spectrum of methodologies. Early approaches relied on hand-crafted structural descriptors, such as degree centrality, clustering coefficients, and motif counts, that were pre-computed and then fed into standard classifiers. A more principled family of methods, known as graph kernels, extended the idea of kernel machines to structured data by defining similarity functions over pairs of graphs or node neighbourhoods~\cite{kriege2020survey}. While both families have proven useful, they suffer from a common limitation: the feature extraction process is decoupled from the learning objective, preventing the model from adapting its graph representations to the task at hand.

Modern GML overcomes this limitation through end-to-end learned representations. These methods are typically organised around three levels of prediction granularity. \textit{Node-level} tasks assign a label or a continuous value to each vertex; canonical examples include community detection and entity classification. \textit{Graph-level} tasks predict a property of an entire graph, as in molecular property prediction or graph classification. \textit{Edge-level} tasks, finally, operate on pairs of nodes connected by an edge and include both link prediction, that is inferring which connections are likely to form, and edge classification, which is precisely the setting adopted in this dissertation for the detection of fraudulent transactions.

Across all these tasks, Graph Neural Networks (GNNs) have emerged as the dominant paradigm. Their success stems from a principled inductive bias: rather than learning a fixed function of global graph statistics, GNNs propagate and transform information along the edges of the graph through an iterative message-passing mechanism. This endows them with the ability to produce node representations that are sensitive to local neighbourhood structure while remaining equivariant to node permutations, a property that no tabular model can achieve. Furthermore, parameters are shared across all nodes and edges, making GNNs naturally scalable to graphs with millions of vertices. When node and edge attributes are available, as is the case in the financial graphs considered here, GNNs can seamlessly fuse topological signals with rich feature information, yielding representations that are far more expressive than either source alone~\cite{wu2020comprehensive}.
