...
class GAGNNLoss(nn.Module):
    def __init__(self, c1=1.0, c2=1.0, c3=1.0, laundry_weight=1.0, task='binary'):
        """
        Args:
    ...
    def forward(self, p_node, y_node, p_trans, y_trans, p_group, y_group,
        ...
        Args:
            ...
            p_trans: Predicted transaction probabilities (or logits for multiclass)
        ...
        if node_mask is not None:
            if node_mask.any():
                l_node = F.binary_cross_entropy_with_logits(p_node[node_mask], y_node[node_mask], pos_weight=pos_weight if self.task == 'binary' else None)
        else:
            l_node = F.binary_cross_entropy_with_logits(p_node, y_node, pos_weight=pos_weight if self.task == 'binary' else None)
        ...
        if group_mask is not None:
            if group_mask.any():
                l_group = F.binary_cross_entropy_with_logits(p_group[group_mask], y_group[group_mask], pos_weight=pos_weight if self.task == 'binary' else None)
        else:
            l_group = F.binary_cross_entropy_with_logits(p_group, y_group, pos_weight=pos_weight if self.task == 'binary' else None)
